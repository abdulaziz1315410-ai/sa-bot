...
// -------------------- Small talk (manual replies) --------------------

function detectSmallTalk(s: string) {
  const t = norm(s);
  ...
  // very short “ok” family → prompt them gently
  const okays = new Set([
    "ok",
    "okay",
    "k",
...
function detectStyle(lastUser: string) {
  ...
  if (lu.includes("be direct") || lu.includes("no bs")) return "HARD";
  if (lu.includes("decide") || lu.includes("choose")) return "DECISION";
  if (lu.includes("worst") || lu.includes("ruined")) return "INTERRUPT";
...
export async function POST(req: Request) {
  try {
    ...
    const lastUser = messages.filter((m) => m.from === "user").pop()?.text || "";

    // ✅ Manual small talk replies (professional + friendly)
    const smallTalk = detectSmallTalk(lastUser);

    if (smallTalk === "GREETING") {
      return NextResponse.json({
        reply:
          "Hi 👋 Tell me what you’re overthinking about, and I’ll turn it into clear steps.",
      });
    }
    ...
    if (smallTalk === "OK") {
      return NextResponse.json({
        reply:
          "Got it. What’s the main thought you keep replaying right now? (One sentence is enough.)",
      });
    }
    ...
    if (smallTalk === "THANKS") {
      return NextResponse.json({
        reply:
          "You’re welcome 🙂 Anything else you want help with about overthinking?",
      });
    }
